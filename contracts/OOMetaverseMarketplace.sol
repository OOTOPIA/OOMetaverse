...
import "./OOMetaverse.sol";
import "./PaymentGateway.sol";

contract OOMetaverseMarketplace is Ownable {

  OOMetaverse private _OOMetaverse;
  ...
  constructor(address _OOMetaverseContractAddress, address _PaymentGatewayAddress, address payable _publisherWallet) {
    _setOOMetaverseContract(_OOMetaverseContractAddress);
  ...
  function _setOOMetaverseContract(address _OOMetaverseContractAddress) private onlyOwner{
    _OOMetaverse = OOMetaverse(_OOMetaverseContractAddress);
  }

  function setOffer(uint256 price, uint256 tokenId, address tokenAddress) public{
    require(_OOMetaverse.ownerOf(tokenId) == msg.sender, "Only the owner of the artwork is allowed to do this");
    require(_OOMetaverse.isApprovedForAll(msg.sender, address(this)) == true, "Not approved to sell");
    ...
    emit artworkAdded(address(_OOMetaverse), msg.sender, price, tokenId, offerId, false);
  }
  ...
  function buyArt(uint256 tokenId, address tokenAddress) public payable{
    ...
    _OOMetaverse.safeTransferFrom(offer.seller, msg.sender, tokenId);
  ...
  function _distributeFees(uint256 tokenId, uint256 price, address payable seller) internal{
    uint8 creatorRoyalty = _OOMetaverse.getRoyalty(tokenId);
    ...
    address payable creator = _OOMetaverse.getCreator(tokenId);
