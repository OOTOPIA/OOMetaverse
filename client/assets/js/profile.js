...
function twitterShareProfile(){
  ...
  let tweet = `https://twitter.com/intent/tweet?text=Check%20out%20this%20artist%20on%20OOMetaverse!&hashtags=oometaverse%2Cbsc%2Cnonfungible%2Cdigitalasset%2Cnft&via=oometaverse&url=${page}`;
  $('#twitterShare').click(()=>{
...
function facebookShareProfile(username){
  ...
  let height = screen.height / 3;
  let page = window.location.href;
  let post = `https://www.facebook.com/sharer/sharer.php?u=${page}%3F&quote=Check%20out%20${username}'s%20NFTs%20on%20OOMetaverse`;
  $('#facebookShare').click(()=>{
...
function emailShareProfile(username){
  let page = window.location.href;
  $('#emailShare').click(()=>{
    window.location.href = `mailto:user@example.com?subject=${username}'s%20profile%20on%20OOMetaverse&body=Check%20out%20the%20artwork%20of%20${username}%20on%20OOMetaverse,%20${page}`;
  });
};
