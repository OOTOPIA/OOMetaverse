...
const ooMetaverseTokenAddress = "0x7Fe11CDf735Be47c0bE9bF7ca3347D6615aE420C";
const ooMetaverseMarketplaceAddress = "0x01e9C9293fAd857423660B948C660fcd895fe5ad";
let ooMetaverseTokenInstance;
let ooMetaverseMarketplaceInstance;
let web3;
let ethPrice;

$(document).ready(async function(){
  web3 = await Moralis.Web3.enable();
  ooMetaverseTokenInstance = new web3.eth.Contract(abi.OOMetaverseToken, ooMetaverseTokenAddress);
  ooMetaverseMarketplaceInstance = new web3.eth.Contract(abi.OOMetaverseMarketplace, ooMetaverseMarketplaceAddress);
...
$('#forSale').click(async ()=>{
  ...
  let activeCount = ifOfferDetailsDuplicatesRemoved.filter(item => !item.isSold && item.active).length;
  if(activeCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork for sale on OOMetaverse, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
  recentlyPutForSale();
});
...
$('#notForSale').click(async ()=>{
  ...
  let inactiveCount = inactiveArtwork.filter(item => !item.active).length;
  if(inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork not for sale on OOMetaverse, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
...
async function viewAll(){
  ...
  if(activeCount < 1 && inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork on OOMetaverse, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
  recentlyPutForSale();
  recentlyMintedAndNotOnSale();
};
...
function soldCardShareOptions(tokenAddress, id){
  ...
  let tweet = `https://twitter.com/intent/tweet?text=Check%20out%20this%20NFT%20on%20OOMetaverse!&hashtags=oometaverse%2Cbsc%2Cnonfungible%2Cdigitalasset%2Cnft&via=oometaverse&url=${tokenPage}`;
  let post = `https://www.facebook.com/sharer/sharer.php?u=${tokenPage}%3F&quote=Check%20out%20this%20NFT%20on%20OOMetaverse`;
  ...
  $('#soldCardEmailBtnInModal' + tokenAddress + id).click(()=>{
    window.location.href = `mailto:user@example.com?subject=Check%20out%20this%20NFT%20on%20OOMetaverse&body=Never%20seen%20anything%20quite%20like%20this,%20${tokenPage}`;
  });
};
...
async function checkForApprovalBeforePuttingOnSale(tokenAddress, id){
  try{
    let approved = await ooMetaverseTokenInstance.methods.isApprovedForAll(user.attributes.ethAddress, ooMetaverseMarketplaceAddress).call();
...
function setApprovalIfNeeded(tokenAddress, id){
  $('#setApprovalBtn' + tokenAddress + id).click(async() =>{
    ...
    await ooMetaverseTokenInstance.methods.setApprovalForAll(ooMetaverseMarketplaceAddress, true).send({from: user.attributes.ethAddress}, (err, txHash) => {
      if(err){
        alert(err.message);
...
function putOnSale(tokenAddress, id, royalty, creator){
  $('#putOnSaleBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await ooMetaverseMarketplaceInstance.methods.setOffer(amountInWei, id, tokenAddress).send({from: user.attributes.ethAddress});
...
function removeFromSale(tokenAddress, id, royalty, creator){
  ...
  $('#removeFromSaleBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await ooMetaverseMarketplaceInstance.methods.removeOffer(id, tokenAddress).send({from: user.attributes.ethAddress});
...
function changePriceFrontEnd(tokenAddress, id, royalty, creator){
  $('#changePriceBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await ooMetaverseMarketplaceInstance.methods.changePrice(amountInWei, id, tokenAddress).send({from: user.attributes.ethAddress});
...
function transferToken(tokenAddress, id){
  $('#transferTokenBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await ooMetaverseTokenInstance.methods.safeTransferFrom(user.attributes.ethAddress, toAddress, id).send({from: user.attributes.ethAddress});
