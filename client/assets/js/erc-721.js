...
async function mint(nftMetadataPath, nftPath){
  ...
  try {
    let receipt = await ooMetaverseTokenInstance.methods.createArtwork(nftMetadataPath, royalty).send({from: user.attributes.ethAddress});
    $('#saveToIPFSBtn').prop('disabled', true);
...
async function uploadToDB(tokenId, nftMetadataPath, nftPath){
  $('#saveToIPFSBtn').html(`Finishing upload to OOMetaverse <div class="spinner-border spinner-border-sm text-light" role="status">
                                                            <span class="sr-only">Loading...</span>
                                                          </div>`);
  try {
    ...
    artwork.set('tokenAddress', ooMetaverseTokenAddress.toLowerCase());
    ...
    console.log(err);
    alert("Error uploading to oometaverse");
    $('#saveToIPFSBtn').prop('disabled', false);
    $('#saveToIPFSBtn').html("Upload and Mint");
  }
};
...
function setArtForSale(tokenId){
  $('#setOffer').click(async() =>{
    ...
    try{
      await ooMetaverseMarketplaceInstance.methods.setOffer(amountInWei, tokenId, ooMetaverseTokenAddress).send({from: user.attributes.ethAddress});
